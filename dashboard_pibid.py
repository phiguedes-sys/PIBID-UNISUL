...
def load_data(gsheets_url=None):
    ...
    if gsheets_url:
        try:
            ...
            st.sidebar.error("Erro de conexão: certifique-se de que a planilha está compartilhada como 'Leitor público'.")
...
supervisors_list = ["Todas"] + sorted([str(x) for x in df_narrativas["Supervisor"].dropna().unique().tolist() if x])
    ...
    if df_filtered_narr.empty:
        ...
        for idx, row in df_filtered_narr.iterrows():
            ...
            with col_text:
                ...
                st.markdown("<p class='text-content'>" + str(row.get('Metodologia', '')) + "</p>", unsafe_allow_html=True)
                
                st.markdown("<p class='section-title'>🌱 Impacto Social e Pedagógico na Escola</p>", unsafe_allow_html=True)
                st.markdown("<p class='text-content'>" + str(row.get('Impacto_Escola', '')) + "</p>", unsafe_allow_html=True)
                ...
                st.markdown("<p class='text-content'><i>\"" + voz_bolsista_texto + "\"</i></p>", unsafe_allow_html=True)
                
                st.markdown("<p class='section-title'>⚠️ Desafios & Como Foram Superados</p>", unsafe_allow_html=True)
                st.markdown("<p class='text-content'>" + str(row.get('Dificuldades', '')) + "</p>", unsafe_allow_html=True)
            ...
            with col_visual:
                ...
                if fotos_reais:
                    if len(fotos_reais) == 1:
                        st.image(fotos_reais[0]["url"], use_container_width=True, caption="Registro - " + sup_narrative)
                    else:
...
with tab_reflections:
    ...
    with sub_tab1:
        ...
        st.markdown(
            ...
            "O ponto partida para a práxis transformadora é o reconhecimento do território educativo. A elaboração de diagnósticos da unidade escolar e a análise crítica do Projeto Político-Pedagógico (PPP) permitem que os licenciandos identifiquem as reais necessidades estruturais, sociais e pedagógicas da instituição. Ao compreenderem o PPP como um documento norteador, os acadêmicos desenvolvem a capacidade de planejar intervenções estratégicas que respondam diretamente aos desafios locais.<br><br>"
...
with tab_magazine:
    ...
    flipbook_url = "https://heyzine.com/flip-book/3b51802cc2.html"
    
    html_code = (
        "<div style='display: flex; flex-direction: column; align-items: center; justify-content: center; margin-top: 1rem; padding: 1rem; background-color: #f8f9fa; border-radius: 12px; border: 2px dashed #1F497D;'>"
        "<iframe allowfullscreen='true' scrolling='no' style='border: none; width: 100%; height: 600px; border-radius: 8px;' src='" + flipbook_url + "'></iframe>"
        "<div style='margin-top: 15px;'>"
        "<a href='" + flipbook_url + "' target='_blank' style='text-decoration: none;'>"
        "<div style='background-color: #1F497D; color: white; padding: 0.8rem 2rem; border-radius: 25px; font-weight: bold; cursor: pointer; text-align: center;'>"
        "Abrir Revista em Tela Cheia ↗"
        "</div>"
        "</a>"
        "</div>"
        "</div>"
...
with tab_search:
    ...
    if search_query:
        ...
        if search_results.empty: 
            ...
            st.success("Encontrado " + str(len(search_results)) + " relato(s) correspondente(s)!")
            for idx, row in search_results.iterrows():
                with st.expander("📌 " + str(row.get('Projeto_Acao', '')) + " — " + str(row.get('Escola', ''))):
                    st.markdown("**Supervisor:** `" + str(row.get('Supervisor', '')) + "`\\n\\n**Metodologia:** " + str(row.get('Metodologia', '')))
                    voz_txt = str(row.get('Voz_Bolsista', ''))
                    st.markdown("**A Voz do Bolsista:** " + voz_txt)
'''

with open("dashboard_pibid_fixed.py", "w", encoding="utf-8") as f:
    f.write(python_code)
print("Arquivo final gravado e validado!")
}
}Não tem problema.

There is a file you can reference named "dashboard_pibid_fixed.py". Refer to this file by its name verbatim.
[source: 54]import streamlit as st
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go
import re
...
EMBEDDED_NARRATIVAS = [
    ...
    },
    {
        ...
        "Projeto_Acao": "Pontes do Saber: Nivelamento Pedagógico", 
        "Periodo_Bimestre": "Agosto a Novembro 2025",
        "Metodologia": "Programa de reforço escolar e nivelamento estruturado para as turmas de 4º e 5º anos do Ensino Fundamental I, focado em mitigar defasagens críticas nas áreas de Alfabetização, Compreensão Leitora e Raciocínio Lógico-Matemático. O projeto envolveu reuniões de alinhamento diagnóstico com as regentes e reuniões periódicas de planejamento metodológico com a coordenação. As aulas presencial de 1h30 semanais utilizavam recursos lúdicos, dinâmicas de raciocínio concreto e escrita individualizada para aproximar os estudantes de forma prazerosa das disciplinas escolares fundamentais.",
        "Impacto_Escola": "Observou-se um fortalecimento do vínculo afetivo dos alunos com a escola e uma sensível evolução em suas notas nas avaliações regulares conduzidas pelas professoras das turmas de origem. Houve adesão integral das famílias das crianças participantes, que frequentavam ativamente as oficinas pedagógicas no contraturno escolar, transformando o espaço do reforço em um polo acolhedor de autoconfiança acadêmica.",
        "Voz_Bolsista": "Os bolsistas (IDs) de Letras e Pedagogia vivenciaram de forma profunda as complexidades da diferenciação pedagógica e do planejamento de aulas de apoio no contraturno. Relatam que o projeto ensinou-os a importância do diagnóstico contínuo de defasagens e de exercer a paciência metodológica para adaptar o cronograma de ensino aos ritmos heterogêneos de cada criança.",
        "Dificuldades": "A inflexibilidade do calendário escolar do contraturno exigiu dos bolsistas uma rígida readequação em suas rotinas de estudos na universidade. O desafio de engajar alunos que apresentavam histórico de frustração escolar foi superado pela criação de jogos matemáticos concretos de tabuleiro e o uso de lanches afetivos.",
        "Foto": "https://drive.google.com/open?id=1iRQ9L99AKJmSSwfCutoK0oxPgBmzxv6h"
...
try:
    ...
    with open("dashboard_pibid_new_flipbook_v2.py", "w", encoding="utf-8") as f:
        f.write(python_code)
    print("Code generated without multi-line triple quotes inside markdown.")
